...
class TestTopologyEndpoint:
    ...
    def test_project_tag_scopes_to_only_the_tagged_bucket(self, client):
        ...
        # as Project=ShiftCommute and cloudres-fintech-logs as
        # Project=CloudResilienceVisualizer.
        response = client.get("/api/topology?project_tag=Project=ShiftCommute")
        data = response.json()
        s3_ids = {n["id"] for n in data["nodes"] if n["type"] == "s3_bucket"}
...
class TestFindingsEndpoint:
    ...
    def test_project_tag_scopes_findings_to_tagged_resources_plus_account(self, client):
        # Phase 9a Feature 1. Scoped to ShiftCommute: the 7 S3
        ...
        response = client.get("/api/findings?project_tag=Project=ShiftCommute")
        data = response.json()
    ...
    def test_findings_have_framework_references_from_all_five_frameworks(self, client):
        ...
        data = response.json()
        expected = {
            "nis2", "ncsc_caf", "mitre_attack", "cyber_essentials", "shiftcommute",
        }
