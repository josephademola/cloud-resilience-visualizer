...
class TestNormalizeEndToEnd:
    ...
    def test_account_node_has_all_five_account_level_misconfigs(self):
        # The mock's account-level misconfigs: root has active access
        # keys, MFA is not enabled, no password policy is configured,
        # no CloudTrail trail exists, and account-level S3 Public
        # Access Block was never configured.
        ...
        assert accounts[0]["properties"]["account_s3_block_public_access_enabled"] is False
...
class TestTagBasedFiltering:
    ...
    uploads' as Project=ShiftCommute and 'cloudres-fintech-logs' as
    Project=CloudResilienceVisualizer. Scoping to Project=ShiftCommute
    ...
    def test_scoping_to_shiftcommute_keeps_only_the_tagged_bucket(self):
        tagged_arns = get_tagged_resource_arns(
            RAW_AWS_DATA, "Project", "ShiftCommute"
        )
    ...
    def test_scoping_to_shiftcommute_drops_untagged_kms_and_iam_resources(self):
        tagged_arns = get_tagged_resource_arns(
            RAW_AWS_DATA, "Project", "ShiftCommute"
        )
        scoped = filter_topology_by_tag(TOPOLOGY, tagged_arns)
    ...
    def test_scoping_to_shiftcommute_still_keeps_the_account_node(self):
        tagged_arns = get_tagged_resource_arns(
            RAW_AWS_DATA, "Project", "ShiftCommute"
        )
        scoped = filter_topology_by_tag(TOPOLOGY, tagged_arns)
