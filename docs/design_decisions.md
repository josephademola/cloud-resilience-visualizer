...
Decision | `cis_aws_foundations.json` maps 13 of this project's 25 finding types to a real CIS v5.0.0 requirement number (an earlier draft targeted v3.0.0 before being corrected to the actual current version — see the file's own `mapping_rationale` for why, and for the exact renumbering between the two). The other 12 have no top-level entry in the file at all — the same "absent, not an empty placeholder" convention `confidential_controls.json` already established for `RESOURCE_MISSING_TAGS` and `GOV-01` — with an `audit_notes` entry for each one explaining specifically why no CIS control applies.
